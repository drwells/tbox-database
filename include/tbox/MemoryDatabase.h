//
// File:        $URL: file:///usr/casc/samrai/repository/SAMRAI/tags/v-2-4-4/source/toolbox/database/MemoryDatabase.h $
...
public:
    ...
    * or came from a default key value (default).
    * is specified, then data is written to stream pout.
    *
    * @warning Unlike SAMRAI, this fork has no default logging object so an
    * input must be specified here.
    */
   virtual void printClassData(std::ostream& os);
    ...
    * input must be specified here.
    */
   void printUnusedKeys(std::ostream& os) const;
    ...
    * input must be specified here.
    */
   void printDefaultKeys(std::ostream& os) const;

private:
   MemoryDatabase(const MemoryDatabase&);       // not implemented
   void operator=(const MemoryDatabase&);       // not implemented
